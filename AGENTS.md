...
- Match changes with focused tests, output snapshots, `doc-sync`, built smokes or real-API e2e.
- Run broad or repeated checks only on request, for CI diagnosis, or for repository-wide changes; CI covers exhaustive and platform checks.
- `test:coverage`, not `test`, is the CI coverage gate ([why](docs/testing.md)).
- After syncing `local-use` with upstream, run the complete `pnpm run build`.
- **Protected Web ports:** `3079`/`3080` are user-managed; test with another loopback port and isolated home ([owner](.agents/skills/dsh-plugin-development/SKILL.md#protected-web-ports-and-live-verification)).
- **Browser/GIF picker:** use `pnpm dsh web --patch apps/web/tests/pin-browse-picker.overlay.yml` for in-page selection; omit only when testing the native picker.
