...
When an agent is idle, has no competing queued work, and its current goal is `active` plus `armed`, the driver also requires that the exact agent owns no ongoing background work. It reads optional `ctx.jobs.hasActive(agent)` and `ctx.subagents.hasPendingContinuations(agent)`: only that owner's `running`/`stopping` Jobs and live direct-child continuable Activations suppress a new reservation. Terminal Jobs, other owners, unowned Jobs, and cold persisted children do not. The driver checkpoints pending goal mutations and rechecks every predicate after the await. If `roundsStarted` already equals `maxGoalRounds`, it records `blocked` with code `round-limit`. Otherwise it reserves the exact identity `{ goalId, revision, round: roundsStarted + 1 }` and the complete rendered prompt before calling `Agent.followup()` with `GoalMessageSource`. The prompt JSON-quotes the objective so multiline or tag-like text remains an unambiguous data value inside the familiar frame.

The `agent/pre-step` waterfall is the entry fence. A positive goal source enters only when it exactly matches the driver's pending identity and content, the live goal still has that id and revision, activation remains armed, the exact agent still owns no ongoing background work, and the round is still the next number. The plugin checks once before delegating and again after downstream listeners return. This second check prevents an async listener from editing or pausing the goal, or starting owned background work, while still entering the old prompt.
...
- **Use an external `agent/pre-step` veto for background waiting** — rejected because the built-in driver must own reservation admission and its rejection paths distinguish stale reservations from policy failures; a listener veto could turn legitimate waiting into a blocker or corrupt the existing race accounting.
...
- Background-work state stays in the Jobs and Subagent capability seams; the driver consumes exact-owner read APIs and does not own job or Activation records.
