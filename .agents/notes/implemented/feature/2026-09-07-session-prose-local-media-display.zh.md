...
`ui-primitives` 在 `MarkdownText` 上拥有 `MarkdownPathImages` 与可选的 `MarkdownImageRenderer` 词表。与 `fileMentions` 一样，它们只在消息稳定后生效，使冻结的流式块无法缓存处理函数。路径词表把本地目标重写为绝对 `http(s)`、`blob` 或 `data` URL；图片渲染器可以把可显示来源替换为持有方提供的呈现。没有路径词表时，本地目标保留静态 alt 文本。加载失败会把图片替换为作者提供的 alt 文本；alt 为空时显示原始目标路径。

`ui-chat` 通过 `AssistantMarkdown` 提供页面稳定的 `localPathMediaUrl` 词表，把绝对 POSIX 路径或 Windows 盘符路径映射到页面同源的 `/api/file?path=…`，并通过可选的 Chat 图片呈现 slot 处理稳定后的本地图片来源。相对路径、协议相对路径、`file://` 目标以及非 HTTP 页面传输保持静态回退。
...
客户端词表支持 Windows 盘符路径。只有提供 Chat 图片 slot 占位的组合才提供灯箱呈现；其他 Markdown 消费方继续直接渲染图片，音视频 Markdown 节点也不渲染播放器。这些属于渲染器限制，与文件路由可读的 MIME 类型无关。

已归档的[模型可读图片路径](../../archived/feature/2026-08-21-model-readable-image-paths.md)记录拥有模型侧行为；本记录拥有用户侧展示，不取代它。

## Testing

渲染器测试覆盖稳定与流式门禁、引用式图片、协议复查、加载失败回退、来源替换和持有方呈现。聊天测试覆盖路径词表、本地图片呈现回调与组件连接。`apps/web/tests/markdown-images.e2e.ts` 浏览器场景使用已播种 Session 启动交付的 Web 组合，检查实际加载与回退文本。模型驱动的记录 Session 往返仍独立于此 UI 期望；后续记录说明当前路由覆盖。
