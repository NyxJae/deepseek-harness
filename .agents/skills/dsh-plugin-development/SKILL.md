...
- Forked DSH installation checkout: `C:\Users\HJ\.dsh\plugins\deepseek-harness`. Make Git-project changes in an assigned `.worktree/<purpose>` within that fork; preserve unrelated changes in the installation root. A running globally linked CLI still resolves its own installed paths until rebuilt or reloaded.
- New or profile-local plugins: `$DSH_HOME/plugins`. Compose them through the active profile instead of editing shipped source.

Keep the fork's upstream remote. Validate built-in changes in the assigned worktree before requesting approval for each commit and push. Do not mix a forked built-in replacement with the installed implementation in one composition.
...
For new local behavior, develop under `$DSH_HOME/plugins` and compose through the active profile. For a requested built-in change, use the approved fork's assigned worktree and verify its package resolution in an isolated profile. A globally linked `dsh` CLI points at its installed checkout, not automatically at another worktree; removing stale profile `link:` or replacement rows applies only after confirming the resolved built-in row. Use a profile replacement only when the active runtime is an installed package or an external plugin intentionally replaces a shipped row. Preserve unrelated built-ins, vendored sources, and the upstream checkout.
