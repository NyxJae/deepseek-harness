...
export const CLIENT_SLOT_API: readonly ClientSlotEntry[] = [
  ...
  {
    key: 'conversation.message.markdown-image',
    kind: 'single',
    scope: 'session',
    summary: 'Renderer for one settled Markdown image after its source passes the Markdown protocol and local-path policy.',
    doc: 'Renderer for one settled Markdown image after its source passes the\nMarkdown protocol and local-path policy. Without an occupant, the default\nMarkdown image element remains in use.',
    registerOptions: [],
    ownerProps: [
    ...
    declaredBy: 'an entry in \'conversation.chat.node\' (client-ui-chat), so it exists while that entry is mounted',
    occupants: [
      'client-ui-attachment MarkdownImage',
    ],
    replaceRisk: 'shadows-shipped-ui',
