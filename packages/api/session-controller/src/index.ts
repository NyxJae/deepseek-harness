...
export class SessionController extends TypertRemoteService {
  ...
  constructor(ctx: Context, config: Config, internals: SessionControllerInternals = {}) {
    ...
    const localMarkdownImages = config.localMarkdownImages ?? { mode: 'disabled' }
    this.history = new SessionHistoryController(
      ctx,
      (observation) => { this.promote(observation) },
      localMarkdownImages.mode !== 'disabled',
    )
    this.markdownImages = new SessionMarkdownImageResolver(ctx, this.agents, localMarkdownImages)
