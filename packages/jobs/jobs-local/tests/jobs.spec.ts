...
describe('LocalJobRegistry.hasActive', () => {
  it('matches exact owners and keeps stopping jobs active until settlement', async () => {
    const ctx = await harness()
    const owner = stubAgent(ctx, 'has-active-owner')
    const other = stubAgent(ctx, 'has-active-other')
    const detachOwner = ctx.agents.register(owner)
    const detachOther = ctx.agents.register(other)
    const task = producer({ owner })
    const id = ctx.jobs.start(task.spec)
    expect(ctx.jobs.hasActive(owner)).toBe(true)
    expect(ctx.jobs.hasActive(other)).toBe(false)
    expect(ctx.jobs.kill(id, owner)).toBe('requested')
    expect(ctx.jobs.hasActive(owner)).toBe(true)
