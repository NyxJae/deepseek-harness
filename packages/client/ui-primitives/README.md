...
| `Menu`, `MenuItemButton` | Dropdown of `items` data rows, separators, and group labels, with nested submenus; `children` adds component rows, each a `MenuItemButton` (`separatorBefore` starts a new group), in the same list. Every row shares the styling, the keyboard walk, and the focus return; closing stays the owner's state change for both kinds. While open, ↑/↓ (with Home and End) walk the list, Tab settles the focused row, and Shift+Tab closes back to the anchor; selecting a row also returns the keyboard to the anchor unless the owner moved it itself. Tab outside the anchor or list stays with browser traversal. Escape closes the menu unless a nested control already handled it, and a closing menu marks Escape handled so an enclosing dialog stays open. `autoFocus` decides solely whether opening focuses the first row. |
...
| `ImageLightbox` | Shared original-image modal with keyboard-accessible controls, bounded zoom, reset, and pointer gestures. |
