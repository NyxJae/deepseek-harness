...
import { IconCloseOutlineRegular, IconFullscreenOutlineRegular, IconPlusOutlineRegular } from './icons/index.tsx'
...
type Gesture =
  | { readonly kind: 'pan'; readonly start: Point; readonly origin: Transform }
...
export function ImageLightbox({ src, alt, labels, onClose }: {
  ...
  const updateTransform = useCallback((next: Transform): void => {
  ...
  useEffect(() => {
    ...
    return () => {
      ...
      // Pointer capture is optional in embedded WebViews and test DOMs.
      // oxlint-disable-next-line typescript/no-unnecessary-condition -- Older runtimes may omit pointer-capture methods.
      for (const pointerId of pointersRef.current.keys()) image?.releasePointerCapture?.(pointerId)
  ...
  }, [onClose, resetTransform, updateTransform, zoomAt])
  ...
  const onPointerEnd = (event: ReactPointerEvent<HTMLImageElement>): void => {
    // Pointer capture is optional in embedded WebViews and test DOMs.
    // oxlint-disable-next-line typescript/no-unnecessary-condition -- Older runtimes may omit pointer-capture methods.
  ...
  const onDoubleClick = (event: ReactMouseEvent<HTMLImageElement>): void => {
    if (transformRef.current.scale > 1.05) resetTransform()
    else zoomAt(2, event.clientX, event.clientY)
  }

  return createPortal(
    <div ref={dialogRef} className={css.backdrop} role="dialog" aria-modal="true" aria-label={labels.dialog}>
      ...
      <div className={css.imageStage}>
        <img
          ...
          alt={alt}
          draggable={false}
          data-zoomed={transform.scale > 1 ? '' : undefined}
