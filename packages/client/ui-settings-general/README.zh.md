...
使用本包可为 dsh Web 客户端提供 Settings 面板、连接恢复控件、功能包导航和逐步进行的首次运行引导。桌面面板最大为 800px × 800px；宽度不超过 640px 时，面板适配安全区视口，分区导航横向滚动，内容独立纵向滚动。面板打开期间，Tab 和 Shift+Tab 的焦点留在面板内；有 Menu 打开时，Escape 先关闭 Menu。用户可以立即重试失败的连接，并在宿主为回环浏览器提供本地配置文件时访问该文件；各功能包提供自己的设置行、分区与引导步骤。
