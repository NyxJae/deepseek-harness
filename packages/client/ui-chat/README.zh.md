...
使用本包可在浏览器中渲染已记录的 Session 对话，包括历史图片、通过可选 child slot 呈现的稳定后本地 Markdown 图片。紧凑显示会收起已完成轮次的过程行，同时保持最终答案和有用上下文可见；已打包的历史 Assistant 连续消息保持收起。本地 transcript（文本记录）与 steering（中途引导）提交会立即显示，并在权威会话记录到达时原子地消失；排队中的提交始终不进入 Chat。本包不组装或修改模型请求。
