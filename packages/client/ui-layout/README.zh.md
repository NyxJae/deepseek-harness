...
本插件在 root slot 中组合侧边栏、主内容和右栏。桌面端侧边栏宽度为 264～420px，默认为 280px，收起后保留 56px 控制栏；窗口宽度低于 1024px 时自动收起为没有 grid track 的 fixed overlay drawer，drawer 宽度独立于中栏／右栏求解，打开右侧面板也会收起手动展开的侧边栏。右侧面板首次打开时使用视口宽度的 45%，之后保留用户的像素宽度偏好，上限为 70%。为给中栏保留 400px，框架先将右侧面板缩减至 300px，再报告空间不足，使占用方将其关闭，最后才进一步压缩中栏。拖动没有过渡延迟；右侧手柄在关闭或全屏时不显示。
...
- **极窄窗口**——右侧面板关闭后，中栏仍可能小于 400px；mobile fixed drawer 不占据 grid track，桌面端关闭 rail 仍为 56px。
