...
describe('publishableImage', () => {
  ...
  it('refuses a target whose real path escapes the repository', () => {
    ...
    const linkedRoot = join(root, 'packages/linked')
    symlinkSync(outside, linkedRoot, 'junction')
    const linkedFile = join(linkedRoot, 'secret.png')

    expect(publishableImage(linkedFile, realpathSync(root))).toBeUndefined()
